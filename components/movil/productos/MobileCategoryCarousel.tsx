...
export function MobileCategoryCarousel({ categories }: MobileCategoryCarouselProps) {
  return (
    ...
    <div className="flex max-w-full snap-x snap-mandatory gap-3 overflow-x-auto scroll-smooth px-0 pb-2 md:hidden">
      {categories.map((category) => (
        ...
        >
          ...
          <span className="block truncate text-[11px] font-black text-zinc-900 dark:text-zinc-100">{category.name}</span>
        </button>
      ))}
    </div>
  );
